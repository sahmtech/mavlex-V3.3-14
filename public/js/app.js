...
function validate_files(files) {
    if (typeof window.mediaUploadConfig === 'undefined') {
        console.error('mediaUploadConfig is not defined.');
        return false;
    }
    var allowed_extensions = $.map(window.mediaUploadConfig.allowedExtensions, function(ext) {
        return ext.toLowerCase();
    });
    var allowed_mime_types = $.map(window.mediaUploadConfig.allowedMimeTypes, function(mime) {
        return mime.toLowerCase();
    });
    var max_file_size = window.mediaUploadConfig.document_size_limit || 5000000; 

    var is_valid = true;

    $.each(files, function(index, file) {
        var extension = file.name.split('.').pop().toLowerCase();
        var mime_type = file.type.toLowerCase();

        if ($.inArray(extension, allowed_extensions) === -1) {
            is_valid = false;
            toastr.error('File type not allowed: ' + file.name);
        }
        // if ($.inArray(mime_type, allowed_mime_types) === -1) {
        //     is_valid = false;
        //     toastr.error('MIME type not allowed: ' + file.name);
        // }

        if (file.size > max_file_size) {
            is_valid = false;
            var size_in_mb = (max_file_size / 1000000).toFixed(2);
            toastr.error('File size exceeds limit (' + size_in_mb + ' MB): ' + file.name);
        }
    });
...
function initialize_file_validation() {
    $('input[type="file"], .docus_notes_media').on('change', function(event) {
        var files = event.target.files;
        if (files.length === 0) {
            return; 
        }
        var is_valid = validate_files(files);
        if (!is_valid) {
            $(this).val('');
        }
    });
}
    initialize_file_validation();
 ...
 if(typeof brands_table !== 'undefined') {
    ...
    '#has_sell_due, #has_sell_return, \
     #has_purchase_due, #has_purchase_return, \
     #has_advance_balance, #has_opening_balance',
    function() {
  ...
  $(document).on('change',
    '#has_no_sell_from, #cg_filter, #status_filter',
    function() {
      contact_table.ajax.reload();
    }
  );
    ...
    $('.user_add_modal').on('shown.bs.modal', function () {
        $('#user_dob').datepicker({
            autoclose: true
        });
    });
    ...
    // Search Settings: collect label texts from label, h4, and p elements within .pos-tab-container
        var label_objects = [];
        var search_options = [{ id: '', text: '' }];
        var i = 0;
        $('.pos-tab-container').find('label, h4, p').each(function(){
            var $elem = $(this);
            var label_text = $elem.text().trim().replace(":", "").replace("*", "");
            if (label_text.length) {
                label_objects.push($elem);
                search_options.push({ id: i, text: label_text });
                i++;
            }
        });
...
$(document).on('click', 'button#delete_invoice', function() {
  ...
  document.querySelectorAll('.toogle-sidebar-submenu > .nav-link')
    .forEach(toggle => {
      toggle.addEventListener('click', e => {

        const parent  = toggle.closest('.toogle-sidebar-submenu');
        ...
        document.querySelectorAll('.toogle-sidebar-submenu.open').forEach(el => {
          el.classList.remove('open');
          const sm = el.querySelector('.side-submenu');
  ...
  document.querySelectorAll('.toogle-sidebar-submenu.open .side-submenu')
    .forEach(sm => (sm.style.display = 'block'));
});
