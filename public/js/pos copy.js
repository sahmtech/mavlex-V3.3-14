...
$(document).ready(function() {
 ...
 /// add by amen
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        //Customer Display
        tr.find('input[name^="item_price"]').val(unit_price_inc_tax.toFixed(2));
...
$(document).on(
    "change",
    ".pos_unit_price, .pos_unit_price_inc_tax, .pos_quantity, .row_discount_amount, .row_discount_type",
...
function updateCurrencyCode() {
    ...
    var totalPayableTargetSpan = document.querySelector('.total-payable-target h3 span:last-child');
