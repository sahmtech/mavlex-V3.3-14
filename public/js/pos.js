...
$(document).ready(function () {
    ...
    if (
        ...
        $("form#add_pos_sell_form").length > 0
    ) {
        initialize_printer();
    }
    ...
    $("select#select_location_id").change(function () {
        ...
        var default_price_group = $(this)
            .find(":selected")
            .data("default_price_group");
        if (default_price_group) {
            if (
                $("#price_group option[value='" + default_price_group + "']")
                    .length > 0
            ) {
        ...
        if ($("#invoice_scheme_id").length) {
            if ($('input[name="is_direct_sale"]').length > 0) {
                ...
                var invoice_scheme_id = $(this)
                    .find(":selected")
                    .data("default_invoice_scheme_id");
            }
        ...
        if ($("#invoice_layout_id").length) {
            let invoice_layout_id = $(this)
                .find(":selected")
                .data("default_invoice_layout_id");
    ...
    $("select#customer_id").select2({
        ...
        language: {
            inputTooShort: function (args) {
                return (
                    LANG.please_enter + args.minimum + LANG.or_more_characters
                );
            },
            noResults: function () {
    ...
    $("#customer_id").on("select2:select", function (e) {
        ...
        $("#advance_balance_text").text(
            __currency_trans_from_en(data.balance),
            true
        );
    ...
    $("table#pos_table tbody").on("change", "select.tax_id", function () {
        ...
        var discounted_unit_price = __get_principle(
            unit_price_inc_tax,
            tax_rate
        );
        var unit_price = get_unit_price_from_discounted_unit_price(
            tr,
            discounted_unit_price
        );
    ...
    $("table#pos_table tbody").on("change", "select.lot_number", function () {
        ...
        if (sub_unit_length > 0) {
            ...
            multiplier = parseFloat(
                select.find(":selected").data("multiplier")
            );
    ...
    $("button#pos-save-card").click(function () {
        ...
        $("input#card_transaction_number_0").val(
            $("#card_transaction_number").val()
        );
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result) {
                    ...
                    var total_paying = __read_number(
                        $("input#total_paying_input")
                    );
    ...
    $(
        "button#submit-sell, button#save-as-draft, button#save-and-print, button#save-and-send"
    ).click(function (e) {
        //Check if product is present or not.
        if ($("table#pos_table tbody").find(".product_row").length <= 0) {
            toastr.warning(LANG.no_products_added);
        ...
        if ($(".enable_cash_denomination_for_payment_methods").length) {
            ...
            if (
                ...
                var payment_amount = __read_number(
                    payment_row.find(".payment-amount")
                );
                var total_denomination = payment_row
            ...
            if (!is_valid) {
                payment_row
                    .find(".cash_denomination_error")
                    .removeClass("hide");
                toastr.error(
                    payment_row.find(".cash_denomination_error").text()
                );
                e.preventDefault();
                return false;
            } else {
    ...
    $("#exchange_rate").change(function () {
        ...
        $("span#total_payable").text(
            __currency_trans_from_en(shown_total, false)
        );
    });
    ...
    $(document).on(
        "change",
        "form#quick_add_product_form input#single_dpp",
        function () {
            var unit_price = __read_number($(this));
            $("table#quick_product_opening_stock_table tbody tr").each(
                function () {
    ...
    $("table#pos_table").on("change", "select.sub_unit", function () {
        ...
        var base_unit_selling_price = tr
            .find("input.hidden_base_unit_sell_price")
            .val();
...
function pos_product_row(
    ...
    if (add_via_ajax) {
        ...
        var is_sales_order =
            $("#sale_type").length && $("#sale_type").val() == "sales_order"
                ? true
                : false;
                ...
                if (result.success) {
                    ...
                    if (result.enable_sr_no == "1") {
                        var new_row = $("table#pos_table tbody")
                            ...
                            .last();
                        new_row
                            .find(".row_edit_product_price_model")
                            .modal("show");
                    }
                    ...
                    $(".pos_product_div").animate(
                        {
                            scrollTop:
                                $(".pos_product_div").prop("scrollHeight"),
                        },
                        1000
                    );
...
function pos_each_row(row_obj) {
    ...
    var min_amount = row_obj
        .find("select.tax_id")
        .find(":selected")
        .data("min_amount");

    var tax_2_rate = row_obj
        .find("select.tax_id_2")
        .find(":selected")
        .data("rate");
    var min_amount_2 = row_obj
        .find("select.tax_id_2")
        .find(":selected")
        .data("min_amount");
    ...
    // Customer Display
    //var rowHtml = row_obj.closest('tr').prop('outerHTML');
    //store_data_local_storage(rowHtml, 'update');
    // store_footer_data();
}
//fix_here_1
...
//         additional_expense += __read_number(
...
function pos_discount(total_amount) {
    ...
    var discount = __calculate_amount(
        calculation_type,
        calculation_amount,
        total_amount
    );
...
//         // Get values, make sure to parse as floats for calculation
//         let unit_price = parseFloat($(this).find(".pos_unit_price").val()) || 0;
//         let unit_price_inc_tax =
//             parseFloat($(this).find(".pos_unit_price_inc_tax").val()) || 0;
//         let quantity = parseFloat($(this).find(".pos_quantity").val()) || 0;

//         // Per row calculations
//         let row_before_tax = unit_price * quantity;
//         let row_after_tax = unit_price_inc_tax * quantity;
//         let row_tax = (unit_price_inc_tax - unit_price) * quantity;

//         // Add to totals
//         total_before_tax += row_before_tax;
//         total_after_tax += row_after_tax;
//         total_tax += row_tax;
//     });

//     // Update the summary fields in the DOM (format as needed)
...
//     // If you have a function to format currency, call it here (e.g., __currency_convert_recursively())
...
// Call it when a row is added/changed/removed:
$(document).on(
    "change",
    ".pos_unit_price, .pos_unit_price_inc_tax, .pos_quantity, .row_discount_amount, .row_discount_type,.tax_id",
    calculateInvoiceTotals
);
$(document).on(
    "input",
    ".pos_unit_price, .pos_unit_price_inc_tax, .pos_quantity, .row_discount_amount",
    calculateInvoiceTotals
);
...
$(document).on(
    "input change",
    "input#shipping_charges, input#additional_expense_value_1, input#additional_expense_value_2, input#additional_expense_value_3, input#additional_expense_value_4",
    function () {
        calculateInvoiceTotals();
    }
);
...
function reset_pos_form() {
    ...
    $("span.total_payable_span", "span.total_paying", "span.balance_due").text(
        0
    );
    ...
    $("input#shipping_details").val(
        $("input#shipping_details").data("default")
    );
    $(
        "input#shipping_address, input#shipping_status, input#delivered_to",
        "input#delivery_person",
        "input#delivery_person_modal"
    ).val("");
...
function set_default_customer() {
    ...
    $("#advance_balance_text").text(
        __currency_trans_from_en(default_customer_balance),
        true
    );
    ...
    if ($("textarea#repair_defects").length > 0 && !customer_set) {
        ...
        if (
            ...
            suggestions = JSON.parse(
                $("input#pos_repair_defects_suggestion").val()
            );
        }
...
function calculate_discounted_unit_price(row) {
    ...
    var row_discount_amount = __read_number(
        row.find("input.row_discount_amount")
    );
    if (row_discount_amount) {
        if (row_discount_type == "fixed") {
            row_discounted_unit_price = this_unit_price - row_discount_amount;
        } else {
            row_discounted_unit_price = __substract_percent(
                this_unit_price,
                row_discount_amount
            );
        }
...
function get_unit_price_from_discounted_unit_price(row, discounted_unit_price) {
    ...
    if (row_discount_amount) {
        if (row_discount_type == "fixed") {
            ...
            this_unit_price = __get_principle(
                discounted_unit_price,
                row_discount_amount,
                true
            );
...
$("table#pos_table tbody").on("change", "input.pos_line_total", function () {
    ...
    tr.find("span.pos_line_total_text").text(
        __currency_trans_from_en(subtotal, true)
    );
...
function getCustomerRewardPoints() {
    ...
    var is_edit =
        $("form#edit_sell_form").length || $("form#edit_pos_sell_form").length
            ? true
            : false;
...
function updateRedeemedAmount(argument) {
    ...
    var amount_per_unit_point = parseFloat(
        $("#rp_redeemed_modal").data("amount_per_unit_point")
    );
    var redeemed_amount = points * amount_per_unit_point;
    $("#rp_redeemed_amount_text").text(
        __currency_trans_from_en(redeemed_amount, true)
    );
...
$(document).on("change", "#rp_redeemed_modal", function () {
    ...
    points = points == "" ? 0 : parseInt(points);
    var amount_per_unit_point = parseFloat(
        $(this).data("amount_per_unit_point")
    );
    var redeemed_amount = points * amount_per_unit_point;
    $("#rp_redeemed_amount_text").text(
        __currency_trans_from_en(redeemed_amount, true)
...
$(document).on("click", "#send_for_service_staff_replacement", function (e) {
    ...
    if (invoice_no) {
        $.ajax({
            method: "get",
            url:
                "/validate-invoice-to-service-staff-replacement/" +
                encodeURI(invoice_no),
            dataType: "json",
            success: function (result) {
                if (result.success == true) {
...
$(document).on(
    "click",
    "#refresh_service_staff_availability_status",
    function () {
...
function update_packing_charge_input() {
    var packing_charge_value = $("#packing_charge_text").text().trim();
    $("#packing_charge_input").val(packing_charge_value);
}
...
function store_data_local_storage(rowHtml, action = "add") {
    ...
    var itemDiscount = __number_uf(
        row.find('input[name^="discount_cd"]').val()
    );
...
function store_footer_data() {
    ...
    var packing_charge = $.trim($("#packing_charge_input").val()) || "0.00";
...
function updateCurrencyCode() {
    ...
    var exchangeCurrencyIdInput = document.getElementById(
    ...
    var totalPayableTargetInput = document.getElementById(
        "final_total_target_input"
    );
    var totalPayableTargetSpan = document.querySelector(
        ".total-payable-target h3 span:last-child"
    );
    var currencyCodeSpan = document.getElementById("currency_code");

    if (!exchangeRateInput) {
        // console.warn('Exchange rate input not found. Currency Exchange Module may be disabled.');
        return;
    }

    if (!exchangeCurrencyIdInput) {
        console.warn(
            "Exchange currency ID input not found. Currency Exchange Module may be disabled."
        );
        return;
    }
    ...
    if (exchangeRateDropdown.value) {
        var selectedOption =
            exchangeRateDropdown.options[exchangeRateDropdown.selectedIndex];
        var exchangeRate = parseFloat(selectedOption.getAttribute("data-rate"));
        var targetCurrencyCode =
            selectedOption.getAttribute("data-target-code");
        var exchangeCurrencyId = selectedOption.getAttribute(
            "data-exchange-currency-id"
        );
        ...
        totalPayableTargetSpan.textContent = __currency_trans_from_en(
            totalPayableInTargetCurrency,
            false,
            false,
            2
        );
...
function updateCurrencyValues() {
    ...
    const exchangeRate =
        parseFloat(document.getElementById("currency_exchange_rate").value) ||
        0;
    ...
    if (exchangeRate > 0) {
        $(".sale_amount_target_label")
            .show()
            .find(".target-currency-label")
            .text("(" + currencyCode + ")");
        ...
        $(
            ".total_payable_target, .total_paying_target, .change_return_target, .balance_due_target, .sale_amount_target"
        ).hide();
    }

    $(".payment-amount").each(function () {
        const baseAmount = parseCurrency($(this).val());
        const targetAmountId = $(this)
            .attr("id")
            .replace("amount", "target_amount");
...
window.addEventListener("productSelected", (event) => {
    console.log("Product Selected event:", event.detail);
    // Since event.detail is an array, access the first element:
