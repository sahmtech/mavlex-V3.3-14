<?php
/**CAUTION DONOT EDIT THIS FILE THE APPLICATION WILL BREAK */
...
class InstallController extends Controller
{
    ...
    public function postDetails(Request $request)
    {
        ...
        try {
            ...
            $input = $request->only(['APP_NAME', 'APP_TITLE', 'DB_HOST', 'DB_PORT', 'DB_DATABASE', 'DB_USERNAME', 'DB_PASSWORD', 'PURCHASE_CODE',
                'BPOS_EMAIL', 'BPOS_USERNAME', 'MAIL_MAILER',
            ...
            $return = pos_boot($input['APP_URL'], __DIR__, $input['PURCHASE_CODE'], $input['BPOS_EMAIL'], $input['BPOS_USERNAME']);
            if (! empty($return)) {
                return $return;
            }
 ...
 public function update(Request $request)
{
    ...
    try {
        ...
        $input = $request->only(['PURCHASE_CODE', 'BPOS_USERNAME', 'BPOS_EMAIL']);
        $return = pos_boot(config('app.url'), __DIR__, $input['PURCHASE_CODE'], $input['BPOS_EMAIL'], $input['BPOS_USERNAME'], 1);
        if (!empty($return)) {
            DB::rollBack(); // Rollback before return
            return $return;
        }
        ...
        if ($version == 1.2) {
            ...
            if (Comparator::greaterThan($this->appVersion, $db_version)) {
                ...
                DB::rollBack(); // Rollback before aborting
                abort(404);
            }
        } else {
            DB::rollBack(); // Rollback before aborting
        ...
        $output = [
            'success' => 1,
            'msg' => 'Updated successfully to version ' . $this->appVersion . ' !!',
        ];
