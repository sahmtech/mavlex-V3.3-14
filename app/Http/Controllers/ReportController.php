...
class ReportController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($contacts)
                ->editColumn('name', function ($row) {
                    ...
                    return '<a href="' . action('ContactController@show', [$row->id]) . '" target="_blank" class="no-print" style="color: #000000;">' .
                            $name .
                        '</a>';
                })
                ->editColumn(
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($request->input('channel'))) {
                $registers->where('cash_registers.channel', $request->input('channel'));
            }
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            ...
            return Datatables::of($registers)
                ...
                ->addColumn(
                    ...
                    '<div class="action-buttons">
                        <button type="button" data-href="{{action(\'CashRegisterController@show\', [$id])}}" class="btn-modal" data-container=".view_register">
                            <img src="' . asset('img/icons/eye.svg') . '" alt="">
                        </button>
                        @if($status != "close" && auth()->user()->can("close_cash_register"))
                            <button type="button" data-href="{{action(\'CashRegisterController@getCloseRegister\', [$id])}}" class=" btn-modal" data-container=".view_register">
            ...
            return Datatables::of($report)
                ...
                ->addColumn('edit', function ($row) {
                    ...
                    if (!empty($row->exp_date)) {
                        $carbon_exp = Carbon::createFromFormat('Y-m-d', $row->exp_date);
                        $carbon_now = Carbon::now();
                        if ( (int) $carbon_now->diffInDays($carbon_exp, false) < 0) {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}">@lang("messages.view")
                    </button> @if(!empty($document))<a href="{{upload_asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>@endif')
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}"><img data-toggle="tooltip" data-placement="top" title="View" src="' . asset('img/icons/eye.svg') . '" alt="">
                    </button> @if(!empty($document))<a href="{{upload_asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>@endif')
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines 
                    as SL', 'SL.id', '=', 'transaction_sell_lines_purchase_lines_v2.sell_line_id')
                ->leftJoin('stock_adjustment_lines 
                    as SAL', 'SAL.id', '=', 'transaction_sell_lines_purchase_lines_v2.stock_adjustment_line_id')
                ...
                ->join('purchase_lines as PL', 'PL.id', '=', 'transaction_sell_lines_purchase_lines_v2.purchase_line_id')
                ...
                ->select(
                    ...
                    'transaction_sell_lines_purchase_lines_v2.quantity as quantity',
                    ...
                    'transaction_sell_lines_purchase_lines_v2.qty_returned',
                    'bl.name as location',
                    'SL.sell_line_note'
                );
    ...
    public function getStockValue(Request $request)
    {
        ...
        $query = $this->productUtil->getProductStockDetails($business_id, $filters, 'export');

        $sub_query = $query->toBase();  
        $stockDetails = DB::query()->fromSub($sub_query, 'sd');
        $closing_stock_by_pp = $stockDetails->sum('stock_price');
        $closing_stock_by_sp = $stockDetails->sum(DB::raw('sd.stock * sd.unit_price'));
    ...
    public function activityLog()
    {
        ...
        if (request()->ajax()) {
            $activities = Activity::with(['subject'])
                                ...
                                ->where('activity_log.subject_type', '!=', 'App\StockCardLine')
                                ->select(
                                    'activity_log.*',
            ...
            return Datatables::of($activities)
                            ...
                            ->editColumn('description', function($row) {
                                $desc = '';
                                if($row->description == 'login'){
                                    $desc .= '<span class="login">' . __('lang_v1.' . $row->description) . '</span>';
                                } else if($row->description == 'logout'){
                                    $desc .= '<span class="logout">' . __('lang_v1.' . $row->description) . '</span>';
                                } else if($row->description == 'edited'){
                                    $desc .= '<span class="email-sent">' . __('lang_v1.' . $row->description) . '</span>';
                                } else if($row->description == 'added'){
                                    $desc .= '<span class="verified">' . __('lang_v1.' . $row->description) . '</span>';
    ...
    public function filter()
    {
        ...
        return view('report.partials.filter')
               ->with(compact('business_locations', 'show_manufacturing_data', 'categories', 'brands', 'units', 'status', 'suppliers', 'customers', 'customer_group', 'payment_types', 'view_stock_filter'));
    }
