...
class BusinessUtil extends Util
{
    ...
    {
        return ['disable_pay_checkout' => 0, 'disable_draft' => 0, 'disable_express_checkout' => 0, 'hide_product_suggestion' => 0, 'hide_recent_trans' => 0, 'disable_discount' => 0, 'disable_order_tax' => 0, 'is_pos_subtotal_editable' => 0 , 'show_product_qty' => 0, 'show_price_check' => 0, 'show_customer_display' => 0, 'disable_currency_exchange'=> 0 ];
    }
    public function defaultDashboardSettings()
    {
        return ['disable_performance' => 0,'disable_sales_due' => 0, 'disable_purchase_due' => 0, 'disable_stock_alert' => 0, 'disable_stock_expiry' => 0, 'disable_sales_order' => 0, 'disable_purchase_order' => 0, 'disable_pending_shipments' => 0, 'disable_payment_recover' => 0 , 'disable_fy_sales_chart' => 0 , 'disable_sell_return_widget' => 0,'disable_purchase_return_widget'=> 0,'disable_expense_widget' => 0,'disable_total_purchase_widget' => 0, 'disable_purchase_due_widget'=>0];
    }
    public function defaultRestaurantSettings()
    {
        return ['enable_kot' => 0, 'enable_order_type' => 0, 'enable_running_orders' => 0, 'enable_restaurant_module' => 0 , 'default_order_type' => '', 'order_type_required' => 0];
    }
    ...
    /**
     * Return the default setting for the invoice.
     *
     * @return array
     */
    public function defaultInvoiceSettings()
    {
        $currencyId = request()->session()->get('business.currency_id');
        $currency = Currency::where('id', $currencyId)->value('id'); // This fetches only the ID directly.
    
        return ['invoice_prefix' => 'INV-', 'currency_id' => $currencyId, 'auto_reminder' => 0, 'auto_due_date' => 365, 'tax_rate_id' => 0, 'default_discount_type' => 'percentage', 'enable_sales_agent' => 1, 'enable_commission_agent' => 1, 'footer_text' => 'Thank you for your business!
        Please Remember all payments to invoices must be made with reference to invoice number to ensure proper account credit.'];
    }

    /**
     * Gives a list of all country codes
     *
     * @return array
     */
    public function allCountryCodes()
    {
        $country_codes = Country::select('id', DB::raw("concat(shortname, ' - ',phonecode) as info"))
            ->orderBy('name')
            ->pluck('info', 'info');
    ...
    {
        $countries = Country::select('id', DB::raw("concat(name) as name"))
            ->orderBy('name')
            ->pluck('name', 'name');
    ...
    /**
     * Show the days remain for the current subscription resource.
     * @return Response
     */
    public function daysRemain($business_id)
    {
        $business = Business::with(['latestSubscription.package'])->find($business_id);

        $details = [
            'business_id' => $business_id,
            'days_left' =>  (int) Carbon::parse(Carbon::now())->diffInDays($business->latestSubscription->end_date),
            'renewal_amount' => $business->latestSubscription->package->renewal_price,
            'package' => $business->latestSubscription->package_details['name']
        ];
