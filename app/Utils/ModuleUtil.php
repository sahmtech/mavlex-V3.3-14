...
//use Modules\Superadmin\Notifications\FreeTrialExpiryNotification;
//use Module;
    ...
    public function isModuleInstalled($module_name)
{
    ...
    if ($is_available) {
        $propertyKey = strtolower($module_name) . '_version';
        $module_version = System::getProperty($propertyKey);
    ...
    public static function expiredResponse($redirect_url = null)
    {
        ...
        $user = \Auth::user();
        //\Notification::send($user, new FreeTrialExpiryNotification());
    ...
    public function moduleSidebar($business_id, $moduleName)
    {
        $commonUtil = new Util();
        $moduleNameFormatted = ucfirst(strtolower($moduleName));
        $moduleNameLower = strtolower($moduleName);
        if ($moduleNameFormatted == 'Superadmin' && !auth()->user()->can('superadmin')) {
            return false;
        }
    
       // $is_admin = $commonUtil->is_admin(auth()->user(), $business_id);
        $moduleAvailable = Module::has($moduleName);
        $moduleInstalled = $this->isModuleInstalled($moduleName);
        $moduleEnabled = (bool) $this->hasThePermissionInSubscription($business_id, $moduleNameLower . '_module');

        
        return $moduleAvailable && $moduleInstalled && $moduleEnabled;
    }
    
    public function sidebarItems(int $businessId): Collection
    {
        $items = collect();

        foreach (Module::allEnabled() as $module) {
            if (! $this->moduleSidebar($businessId, $module->getName())) {
                continue;
            }

            $sidebarCfg = config($module->getLowerName() . '.sidebar', []);
            foreach ($sidebarCfg as $row) {
                $items->push(
                    collect($row)
                        ->merge(['module' => $module->getLowerName()])
                        ->toArray()
                );
            }
        }
