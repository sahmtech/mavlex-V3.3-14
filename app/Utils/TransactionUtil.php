...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $invoice_no = !empty($input['invoice_no']) ? $input['invoice_no'] : $this->getInvoiceNumber($business_id, $input['status'], $input['location_id'], $invoice_scheme_id, $sale_type, $input['payment'][0]['method'] ?? 'cash', $input['prefix'] ?? '');

        $transaction = Transaction::where('business_id', $business_id)->where('invoice_no', $invoice_no)->first();
        if (!empty($transaction)) {
            $invoice_no = Str::random(7);
        }
        ...
        $is_zatca_enabled = app(\App\Utils\ModuleUtil::class)->isModuleInstalled('Zatca');
        ...
        $transaction = Transaction::create([
            ...
            'currency_exchange_rate' => !empty($input['currency_exchange_rate']) ? ($uf_data ? $this->num_uf($input['currency_exchange_rate']) : $input['currency_exchange_rate']) : 0,
            'exchange_currency_id' => !empty($input['exchange_currency_id']) ? $input['exchange_currency_id'] : null,
            ...
            'is_running_order' => !empty($input['is_running_order']) ? 1 : 0,
            'order_type' => !empty($input['order_type']) ? $input['order_type'] : null,
        ...
        if (!empty($is_zatca_enabled)) {
            $transactionData['return_parent_id'] = $input['return_parent_id'] ?? null;
            $transactionData['document_type'] = ($input['document_type'] ?? '') === '1000' ? 'standard' : 'simplified';
        }
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            //Zatca Code
            if (!empty($is_zatca_enabled)) {
                $Main_Tax_Rate = TaxRate::find($product['tax_id']);
                $Main_Tax_Rate = ($Main_Tax_Rate ? $Main_Tax_Rate->amount : 0) / 100;
            }
            $multiplier = 1;
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                //Remove this line: $uf_unit_price = $uf_data ? $this->num_uf($product['unit_price_inc_tax']) : $product['unit_price_inc_tax'];
                ...
                // Zatca Code
                if (!empty($is_zatca_enabled)) {
                    $total_item_tax = (($uf_unit_price_inc_tax * ($uf_quantity * $multiplier)) / ($Main_Tax_Rate + 1)) * $Main_Tax_Rate;
                    $line['total_item_tax'] = $total_item_tax;
                }
        ...
        if (!empty($lines_formatted)) {
            ...
            foreach ($lines_formatted as $key => $value) {
                if (!empty($products_modified_combo[$key]['product_type']) && $products_modified_combo[$key]['product_type'] == 'combo') {
                    // Add combo_modifier_ids for each sub-product in the combo
                    foreach ($products_modified_combo[$key]['combo'] as &$combo_product) {
                        $combo_product['combo_modifier_ids'] = isset($products_modified_combo[$key]['modifiers']) ? json_encode(array_column($products_modified_combo[$key]['modifiers'], 'id')) : json_encode([]);
                    }
    ...
    private function __makeLinesForComboProduct($combo_items, $parent_sell_line)
    {
        ...
        foreach ($combo_items as $value) {
            ...
            $combo_lines[] = new TransactionSellLine([
                ...
                'children_type' => 'combo',
                'combo_modifier_ids' => isset($value['modifier_ids']) ? $value['modifier_ids'] : null
            ]);
        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $is_zatca_enabled = app(\App\Utils\ModuleUtil::class)->isModuleInstalled('Zatca');
        if (!empty($is_zatca_enabled)) {
            $zatca_util = new \Modules\Zatca\Http\Utils\ZatcaUtil();
        }
        ...
        // Zatca Code:
        if (!empty($is_zatca_enabled)) {
            $zs = $location_details->zatcaSetting;
            $output['is_connected'] = (int) optional($zs)->is_connected;
            $output['table_product_label_en'] = $il->module_info['zatca']['table_product_label_en'] ?? '';
            $output['table_qty_label_en'] = $il->module_info['zatca']['table_qty_label_en'] ?? '';
            $output['table_unit_price_label_en'] = $il->module_info['zatca']['table_unit_price_label_en'] ?? '';
            $output['table_subtotal_label_en'] = $il->module_info['zatca']['table_subtotal_label_en'] ?? '';
       
            $info = $location_details->zatca_info ?? [];
            $output['crn_name']  = $info['tax_label_2']  ?? '';
            $output['crn_number'] = $info['tax_number_2'] ?? '';
            $output['vat_label']  = $info['tax_label_1']  ?? '';
            $output['vat_number'] = $info['tax_number_1'] ?? '';
            $output['street_name'] = $info['street_name'] ?? '';
            $output['plot_identification'] = $info['plot_identification'] ?? '';
            $output['city_sub_division'] = $info['city_sub_division'] ?? '';
            $output['city'] = $info['city'] ?? '';
            $output['country'] = $info['country'] ?? '';
            $output['postal_number'] = $info['postal_number'] ?? ''; 
            $output['building_number'] = $info['building_number'] ?? '';            
        }
        ...
        if ($il->show_customer == 1) {
            ...
            //Zatca Code:
            $output['customer_label_en'] = (!empty($il->module_info['zatca']['customer_label_en']) ? $il->module_info['zatca']['customer_label_en'] : '');
            ...
            //Zatca Code:
            $output['customer_tax_label_en'] = (!empty($il->module_info['zatca']['client_tax_label_en']) ? $il->module_info['zatca']['client_tax_label_en'] : '');
        ...
        $output['invoice_no_prefix_en']    = !empty($il->module_info['zatca']['invoice_no_prefix_en']) ? $il->module_info['zatca']['invoice_no_prefix_en']: '';
        ...
        if ($transaction_type == 'sell_return') {
            ...
            //Zatca Code:
            $output['invoice_heading_en']   =  (!empty($il->module_info['zatca']['cn_heading_en']) ? $il->module_info['zatca']['cn_heading_en'] : '');
            $output['invoice_no_prefix_en'] = (!empty($il->module_info['zatca']['cn_no_label_en']) ? $il->module_info['zatca']['cn_no_label_en'] : '');
            $output['parent_invoice_no_prefix_en'] = (!empty($il->module_info['zatca']['invoice_no_prefix_en']) ? $il->module_info['zatca']['invoice_no_prefix_en'] : '');
            ...
            //Zatca Code:
        } elseif ($transaction_type == 'sell_add') {
            $output['invoice_heading']      = __('lang_v1.sell_add');
            $output['invoice_heading_en']   = (!empty($il->module_info['zatca']['cn_heading_en']) ? $il->module_info['zatca']['cn_heading_en'] : '');
            $output['invoice_no_prefix']    = __('lang_v1.cn_no_label_add');
            $output['invoice_no_prefix_en'] = (!empty($il->module_info['zatca']['cn_no_label_en']) ? $il->module_info['zatca']['cn_no_label_en'] : '');
            //Parent sell details(return_parent_id)
            $output['parent_invoice_no']           = Transaction::find($transaction->return_parent_id)->invoice_no;
            $output['parent_invoice_no_prefix']    = $il->invoice_no_prefix;
            $output['parent_invoice_no_prefix_en'] = $il->invoice_no_prefix_en;
            ...
            //Zatca Code
            $output['invoice_heading_en']  = (!empty($il->module_info['zatca']['proforma_heading_en']) ? $il->module_info['zatca']['proforma_heading_en'] : '');
            ...
            //Zatca Code
            $output['invoice_heading_en']   = (!empty($il->module_info['zatca']['quotation_heading_en']) ? $il->module_info['zatca']['quotation_heading_en'] : '');
            $output['invoice_no_prefix_en'] = (!empty($il->module_info['zatca']['quotation_no_prefix_en']) ? $il->module_info['zatca']['quotation_no_prefix_en'] : '');
            ...
            $output['invoice_no_prefix'] = $il->quotation_no_prefix;
            //Zatca Code
            $output['invoice_heading_en']   = !empty($il->common_settings['sales_order_heading_en']) ? $il->common_settings['sales_order_heading_en'] : __('lang_v1.sales_order');
            $output['invoice_no_prefix_en'] = (!empty($il->module_info['zatca']['quotation_no_prefix_en']) ? $il->module_info['zatca']['quotation_no_prefix_en'] : '');
        } else {
            $output['invoice_heading'] = $il->invoice_heading;

            //Zatca Code
            $output['invoice_heading_en'] = (!empty($il->module_info['zatca']['invoice_heading_en']) ? $il->module_info['zatca']['invoice_heading_en'] : '');
            if ($transaction->payment_status == 'paid' && !empty($il->invoice_heading_paid)) {
                $output['invoice_heading'] .= ' ' . $il->invoice_heading_paid;
                //Zatca Code 
                $output['invoice_heading_en'] .= ' ' . (!empty($il->module_info['zatca']['invoice_heading_paid_en']) ? $il->module_info['zatca']['invoice_heading_paid_en'] : '');
                ...
                //Zatca Code
                $output['invoice_heading_en'] .= '  ' . (!empty($il->module_info['zatca'][' invoice_heading_not_paid_en']) ? $il->module_info['zatca']['invoice_heading_not_paid_en'] : '');
            }
        }
        //Zatca Code
        $output['date_label_en'] = (!empty($il->module_info['zatca']['date_label_en']) ? $il->module_info['zatca']['date_label_en'] : '');
        ...
        $output['due_date_label_en'] = (!empty($il->module_info['zatca']['due_date_label_en']) ? $il->module_info['zatca']['due_date_label_en'] : '');
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            //Zatca Code
            $output['item_discount_label_en']  = (!empty($il->module_info['zatca']['item_discount_label_en']) ? $il->module_info['zatca']['item_discount_label_en'] : '');
            $output['item_tax_value_label_en'] = (!empty($il->module_info['zatca']['item_tax_value_label_en']) ? $il->module_info['zatca']['item_tax_value_label_en'] : '');
            $output['tax_summary_label_en']    = (!empty($il->module_info['zatca']['tax_summary_label_en']) ? $il->module_info['zatca']['tax_summary_label_en'] : '');
            ...
            foreach ($details['lines'] as $line) {
                ...
                //zatca code
                if (!empty($is_zatca_enabled)) {
                    $total_line_taxes_qty += $line['total_item_tax_unformatted'] ?? 0;
            ...
            $output['total_quantity_label_en']  = (!empty($il->module_info['zatca']['total_quantity_label_en']) ? $il->module_info['zatca']['total_quantity_label_en'] : '');
            ...
            foreach ($lines as $key => $value) {
                ...
                //Zatca Code
                if (!empty($is_zatca_enabled)) {
                    $total_line_taxes_qty += $value->total_item_tax;
                }
            }
        ...
        $output['discount_label_en'] = (!empty($il->module_info['zatca']['discount_label_en']) ? $il->module_info['zatca']['discount_label_en'] : '');
        $output['discount_label_en'] .= ':';
        $output['tax_label_en'] = (!empty($il->module_info['zatca']['tax_label_en']) ? $il->module_info['zatca']['tax_label_en'] : '');
        $output['line_tax_label_en'] = (!empty($il->module_info['zatca']['tax_label_en']) ? $il->module_info['zatca']['tax_label_en'] : '');
        $output['tax_label_en'] .= ':';
        $output['cat_code_label_en'] = (!empty($il->module_info['zatca']['cat_code_label_en']) ? $il->module_info['zatca']['cat_code_label_en'] : '');
        $output['subtotal_label_en'] = (!empty($il->module_info['zatca']['sub_total_label_en']) ? $il->module_info['zatca']['sub_total_label_en'] : '') . ':';
        $output['round_off_label_en'] = (!empty($il->module_info['zatca']['round_off_label_en']) ? $il->module_info['zatca']['round_off_label_en'] . ':' : __('lang_v1.round_off') . ':');
        if (!empty($is_zatca_enabled)) {
            $output['total_item_tax_qty'] = $this->num_f($total_line_taxes_qty, $show_currency, $business_details);
            $output['total_item_tax_qty_uf'] = $total_line_taxes_qty;
        }
        ...
        if ($transaction_type == 'sell_return') {
            ...
            //Zatca Code
            $output['total_label_en'] = (!empty($il->module_info['zatca']['cn_amount_label_en']) ? $il->module_info['zatca']['cn_amount_label_en'] . ':' : '');
        } else {
            $output['total_label'] = $invoice_layout->total_label . ':';
            $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
            //Zatca Code
            $output['total_label_en'] = (!empty($il->module_info['zatca']['total_label_en']) ? $il->module_info['zatca']['total_label_en'] . ':' : '');
        }
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            //zatca code
            $output['total_due_label_en']  =  $il->module_info['zatca']['total_due_label_en']  ?? '';
            $output['total_paid_label_en'] =  $il->module_info['zatca']['paid_label_en']  ?? '';
        ...
        //Zatca QR CODE for SELL Return
        if (in_array($transaction_type, ['sell_return'])) {
            $output['show_qr_code'] = !empty($il->show_qr_code) ? true : false;
            $zatca_qr = !empty($il->common_settings['zatca_qr']) ? true : false;
            if ($zatca_qr) {
                if (!empty($is_zatca_enabled)) {
                    $total_order_tax = 0;
                    $qr_code_text = $zatca_util->QR($transaction, $total_order_tax);
                } else {
                    $total_order_tax = 0;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                //Zatca Code
                if (in_array('total_tax', $qr_code_fields) && !empty($is_zatca_enabled)) {
                    $total_order_tax_qty = $transaction->tax_amount + $total_line_taxes_qty;
                    $total_order_tax_qty_formatted = $this->num_f($total_order_tax_qty, $show_currency, $business_details);
                    $qr_code_details[] = $is_label_enabled ? __('sale.tax') . ': ' . $total_order_tax_qty_formatted : $total_order_tax_qty_formatted;
                }
            ...
            if (!empty($is_zatca_enabled)) {
                /*
                        ...
                        * 5- VAT Total - Tag = 5
                        * using package github.com/SallaApp/ZATCA
                        * */
                $transaction = Transaction::find($transaction_id);
                ...
                foreach ($transaction->sell_lines as $item) {
                    ...
                    if ($item->line_discount_amount > 0) {
                        $item_row['discounts'] =
                            [
                                ['reason' => 'General Discount', 'amount' => ($item->unit_price_before_discount - $item->unit_price) * 2],
                            ];
                    } else {
                        $item_row['discounts'] = [];
                    }
                ...
                $invoice = [
                    "invoice_id" => $transaction->invoice_no,
                    "invoice_uuid" => $transaction->uuid,
                    "invoice_serial_number" => $transaction->id,
                    "invoice_date" => Carbon::parse($transaction->created_at)->format('Y-m-d'),
                    "invoice_time" => Carbon::parse($transaction->created_at)->format('H:i:s'),
                    "invoice_timestamp" => Carbon::parse($transaction->created_at)->toDateTimeLocalString() . 'Z',
                    "invoice_signing_time" => Carbon::parse($transaction->signing_time ?? '')->toIso8601ZuluString(),
                    "invoice_type" => $transaction->invoice_type ?? '',
                    "invoice_previous_hash" => $transaction->hash,
                    "invoice_billing_reference" => null,
                    "document_type" => '0200000',
                    "client" => null

                ];
                // $new_zatca = new Zatca($business,$items,$invoice);
                // $new_zatca->build_items_rows();
                // $generatedString = $new_zatca->generateQrCode();
        ...
        //Advance Restaurant
        $orderTypes = [
            'dine_in' => trans("lang_v1.dine_in"),
            'takeaway' => trans("lang_v1.take_away"),
            'delivery' => trans("lang_v1.delivery"),
        ];
        $output['prebill'] = !empty($transaction->is_running_order) ? 1 : 0;
        $output['prebill_text'] = trans("lang_v1.prebill");
        $output['order_type'] = !empty($transaction->order_type) ? $orderTypes[$transaction->order_type] : '';
        $output['order_type_label'] = trans("lang_v1.order_type");
        $output['order_label'] = trans("lang_v1.order_label");
        $output['order_notes_label'] = trans("lang_v1.order_notes_label");
        $output['kot_text'] = trans("lang_v1.kot");
        $output['remarks'] = trans("lang_v1.remarks");

        //Currency Exchange Module
        $total_payment = TransactionPayment::where('transaction_id', $transaction->id)
            ->where('is_return', 0)
            ->first();
        $total_payment_return = TransactionPayment::where('transaction_id', $transaction->id)
            ->where('is_return', 1)
            ->first();
        $paid_local_currency = 0;
        if ($total_payment) {
            $paid_local_currency = $total_payment->amount;
        }

        if ($total_payment_return) {
            $return_local_currency = $total_payment_return->amount;
        } else {
            $return_local_currency = 0;
        }
        $exchange_rate = $transaction->currency_exchange_rate;

        if (!empty($transaction->currency_exchange_rate) && $transaction->currency_exchange_rate != 0) {
            $uf_paid_foreign_currency = $paid_local_currency / $exchange_rate;
        } else {
            $uf_paid_foreign_currency = 0;
        }
        if (class_exists(\Modules\CurrencyExchangeRate\Entities\CurrencyExchangeRate::class)) {
            if (!empty($transaction->exchange_currency_id)) {
                $foreign_currency_symbol = \Modules\CurrencyExchangeRate\Entities\CurrencyExchangeRate::find($transaction->exchange_currency_id);
                if ($foreign_currency_symbol) {
                    $target_currency = Currency::find($foreign_currency_symbol->target_currency_id);
                    $output['foreign_currency_symbol'] = $target_currency ? $target_currency->symbol : 'NULL';
                }
            }
        }
        //$foreign_currency = $transaction->exchangeRate->targetCurrency->currency ?? 'NULL';
        //$foreign_currency_symbol = $transaction->exchangeRate->targetCurrency->symbol ?? 'NULL';

        $paid_foreign_currency = $this->num_f($uf_paid_foreign_currency);
        //$output['foreign_currency'] = $foreign_currency;
        // $output['foreign_currency_symbol'] = $foreign_currency_symbol;
        $output['exchange_rate'] = $this->num_f($exchange_rate);
        $output['paid_foreign_currency'] =  $paid_foreign_currency;
        $output['return_local_currency'] =  $this->num_f($return_local_currency);

        //Currency Exchange Labels
        if (class_exists(\Modules\CurrencyExchangeRate\Entities\CurrencyExchangeRate::class)) {
            if (!empty($il->module_info['currencyexchangerate']['show_foreign_currency'])) {
                $output['foreign_currency_label'] = $il->module_info['currencyexchangerate']['foreign_currency_label'];
                $output['foreign_currency'] = '';

                if (!empty($transaction->exchange_currency_id)) {
                    $foreign_currency = \Modules\CurrencyExchangeRate\Entities\CurrencyExchangeRate::find($transaction->exchange_currency_id);
                    if ($foreign_currency) {
                        $target_currency = Currency::find($foreign_currency->target_currency_id);
                        $output['foreign_currency'] = $target_currency ? $target_currency->currency : 'NULL';
                    }
                }
            }
            if (!empty($il->module_info['currencyexchangerate']['show_exchange_rate'])) {
                $output['exchange_rate_label'] = $il->module_info['currencyexchangerate']['exchange_rate_label'];
                $output['exchange_rate'] = $this->num_f($exchange_rate);
            }
            if (!empty($il->module_info['currencyexchangerate']['show_paid_foreign_currency'])) {
                $output['paid_foreign_currency_label'] = $il->module_info['currencyexchangerate']['paid_foreign_currency_label'];
                $output['paid_foreign_currency'] = $paid_foreign_currency;
            }
            if (!empty($il->module_info['currencyexchangerate']['show_return_local_currency'])) {
                $output['return_local_currency_label'] = $il->module_info['currencyexchangerate']['return_local_currency_label'];
                $output['return_local_currency'] = $this->num_f($return_local_currency);
            }
        }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if (!empty($is_zatca_enabled)) {
                $line_array['total_item_tax'] = $this->num_f($line->total_item_tax, false, $business_details);
                $line_array['total_item_tax_unformatted'] = $line->total_item_tax;
            }

            // Field for 3rd column
            $line_array['unit_price_inc_tax'] = $this->num_f($line->unit_price_inc_tax, false, $business_details);
            $line_array['unit_price_inc_tax_uf'] = $line->unit_price_inc_tax;
            $line_array['unit_price_exc_tax'] = $this->num_f($line->unit_price, false, $business_details);
            $line_array['price_exc_tax'] = $line->quantity * $line->unit_price;
            $line_array['unit_price_before_discount'] = $this->num_f($line->unit_price_before_discount, false, $business_details);
            $line_array['unit_price_before_discount_uf'] = $line->unit_price_before_discount;

            // Fields for 4th column
            $line_array['line_total'] = $this->num_f($line->unit_price_inc_tax * $line->quantity, false, $business_details);
            $line_array['line_total_uf'] = $line->unit_price_inc_tax * $line->quantity;

            // Zatca Code:
            if (!empty($is_zatca_enabled)) {
                $line_array['line_total_exc_tax'] = $this->num_f(($line->unit_price_inc_tax * $line->quantity) - $line->total_item_tax, false, $business_details);
                $line_array['line_total_exc_tax_uf'] = ($line->unit_price_inc_tax * $line->quantity) - $line->total_item_tax;
            } else {
                $line_array['line_total_exc_tax'] = $this->num_f($line->unit_price * $line->quantity, false, $business_details);
            ...
            if ($product->type == 'combo' && !empty($variation->combo_variations)) {
                // Fetch combo variations
                $combo_variations = is_string($variation->combo_variations) ? json_decode($variation->combo_variations, true) : $variation->combo_variations;
                $line_array['combo_variations'] = [];
                foreach ($combo_variations as $combo_variation) {
                    $combo_variation_details = Variation::find($combo_variation['variation_id']);
                    if ($combo_variation_details) {
                        $combo_product = $combo_variation_details->product;
                        $combo_line_array = [
                            'name' => $combo_product->name,
                            'variation' => (empty($combo_variation_details->name) || $combo_variation_details->name == 'DUMMY') ? '' : $combo_variation_details->name,
                            'quantity' => $combo_variation['quantity'],
                            'units' => $combo_product->unit ? $combo_product->unit->short_name : '',
                            'unit_price_inc_tax' => $this->num_f($combo_variation_details->sell_price_inc_tax, false, $business_details),
                            'unit_price_exc_tax' => $this->num_f($combo_variation_details->default_sell_price, false, $business_details),
                            'line_total' => $this->num_f($combo_variation_details->sell_price_inc_tax * $combo_variation['quantity'], false, $business_details),
                        ];

                        // Fetch and add modifiers from the correct child records
                        $child_sell_line = \DB::table('transaction_sell_lines')
                            ->where('parent_sell_line_id', $line->id)
                            ->where('variation_id', $combo_variation['variation_id'])
                            ->where('children_type', 'combo')
                            ->first();

                        if ($child_sell_line && !empty($child_sell_line->combo_modifier_ids)) {
                            $combo_modifier_ids = explode(',', $child_sell_line->combo_modifier_ids);
                            $combo_line_array['combo_modifiers'] = [];
                            foreach ($combo_modifier_ids as $combo_modifier_id) {
                                $combo_modifier = Variation::find(trim($combo_modifier_id));
                                if ($combo_modifier) {
                                    $combo_line_array['combo_modifiers'][] = [
                                        'combo_modifier_name' => $combo_modifier->name,
                                        'combo_modifier_price' => $this->num_f($combo_modifier->sell_price_inc_tax, false, $business_details),
                                    ];
                                }
                            }
                        }
                        $line_array['combo_variations'][] = $combo_line_array;
                    }
                }
            }
     ...
     * @return string
     */
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null, $payment_method = 'cash', $prefix = null)
    {
        if ($status == 'final' || $payment_method == 'none') {
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $query->where('trans_date', '>=', $start_date)
                ->where('trans_date', '<=', $end_date);
        }

        if (empty($start_date) && !empty($end_date)) {
            $query->where('trans_date', '<=', $end_date);
        }
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $query->where('transactions.trans_date', '>=', $start_date)
                ->where('transactions.trans_date', '<=', $end_date);
        }

        if (empty($start_date) && !empty($end_date)) {
            $query->where('transactions.trans_date', '<=', $end_date);
        }
    ...
    {
        $key = "getSellsLast30Days" . $business_id . $group_by_location;
    ...
    {
        $key = "getSellsCurrentFy" . $business_id . $start . $end . $group_by_location;
    ...
    public function adjustMappingPurchaseSell(
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            $sell_purchases = Transaction::join('transaction_sell_lines AS SL', 'transactions.id', '=', 'SL.transaction_id')
                ->join('transaction_sell_lines_purchase_lines_v2 as TSP', 'SL.id', '=', 'TSP.sell_line_id')
            ...
            $sell_purchases = Transaction::join('transaction_sell_lines AS SL', 'transactions.id', '=', 'SL.transaction_id')
                ->leftjoin('transaction_sell_lines_purchase_lines_v2 as TSP', 'SL.id', '=', 'TSP.sell_line_id')
                ->where('transactions.id', $transaction->id)
                ->select(
                    'TSP.id as slpl_id',
     ...
     * transaction_sell_lines_purchase_lines_v2 and purchase_lines.quantity_sold
     *
     ...
     * transaction_sell_lines_purchase_lines_v2
    ...
    public function adjustMappingPurchaseSellAfterEditingPurchase($before_status, $transaction, $delete_purchase_lines)
    {
        if ($before_status == 'received' && $transaction->status == 'received') {
            ...
            $purchase_lines = Transaction::join(
                ...
                ->join(
                    'transaction_sell_lines_purchase_lines_v2 AS TSPL',
                    'PL.id',
                    '=',
        ...
        foreach ($purchase_lines as $purchase_line) {
            ...
            //Decrease the quantity from transaction_sell_lines_purchase_lines_v2 or delete it if zero
            $tspl = TransactionSellLinesPurchaseLines::where('purchase_line_id', $purchase_line['id'])
                ->leftjoin(
                    'transaction_sell_lines AS SL',
                    'transaction_sell_lines_purchase_lines_v2.sell_line_id',
                    '=',
                    'SL.id'
                )
                ->leftjoin(
                    'stock_adjustment_lines AS SAL',
                    'transaction_sell_lines_purchase_lines_v2.stock_adjustment_line_id',
                    '=',
                    'SAL.id'
                )
                ->orderBy('transaction_sell_lines_purchase_lines_v2.id', 'desc')
                ->select([
                    ...
                    'transaction_sell_lines_purchase_lines_v2.quantity',
                    'transaction_sell_lines_purchase_lines_v2.purchase_line_id',
                    'transaction_sell_lines_purchase_lines_v2.id as tslpl_id'
                ])
                ->get();
    ...
    public function canBeEdited($transaction, $edit_duration)
    {
        ...
        // Ensure $edit_duration is an integer
        $edit_duration = (int)$edit_duration;
     ...
     * @return float
     */
    public function getOpeningClosingStock(
        int    $business_id,
        string $date,
        ?int   $location_id    = null,
        bool   $is_opening     = false,
        bool   $by_sale_price  = false,
        array  $filters        = []
    ): float {
        // 1) Build your date filter (opening vs closing)
        if ($is_opening) {
            $nextDay = Carbon::parse($date)->addDay()->format('Y-m-d');
            $dateFilter = function($q) use ($date, $nextDay) {
                $q->whereDate('t.transaction_date', '<=', $date)
                  ->orWhere(function($q2) use ($nextDay) {
                      $q2->whereDate('t.transaction_date', $nextDay)
                         ->where('t.type', 'opening_stock');
                  });
            };
        } else {
            $dateFilter = fn($q) => $q->whereDate('t.transaction_date', '<=', $date);
        }
    
        // 2) Product filters helper
        $applyProductFilters = function($q) use ($filters) {
            $q->when($filters['category_id']   ?? null, fn($q,$v)=>$q->where('p.category_id',   $v))
              ->when($filters['sub_category_id'] ?? null, fn($q,$v)=>$q->where('p.sub_category_id',$v))
              ->when($filters['brand_id']       ?? null, fn($q,$v)=>$q->where('p.brand_id',      $v))
              ->when($filters['unit_id']        ?? null, fn($q,$v)=>$q->where('p.unit_id',       $v));
        };
        ...
        //
        // A) PURCHASES & OPENING STOCK
        //
        // Decide whether to value them by purchase‐price or variation’s sale price
        $purchaseExpr = $by_sale_price
            ? 'v.sell_price_inc_tax'
            : '(pl.purchase_price + COALESCE(pl.item_tax,0))';
    
        $purchaseValue = DB::table('purchase_lines AS pl')
            ->join('transactions AS t', 'pl.transaction_id', '=', 't.id')
            ->join('variations   AS v', 'pl.variation_id',   '=', 'v.id')
            ->join('products     AS p', 'pl.product_id',     '=', 'p.id')
            ->when($location_id,     fn($q)=>$q->where('t.location_id',   $location_id))
            ->when($permitted!=='all',fn($q)=>$q->whereIn('t.location_id', $permitted))
            ->where('t.business_id', $business_id)
            ->whereIn('t.type', ['purchase','opening_stock'])
            ->where($dateFilter)
            ->tap($applyProductFilters)
            ->selectRaw("
                SUM(
                  (
                    pl.quantity 
        ...
        //
        // B) SALES (always negative)
        //
        if ($by_sale_price) {
            // Use the sale‐line’s price + tax
            $salesValue = DB::table('transaction_sell_lines AS tsl')
                ...
                ->selectRaw("
                    SUM(
                      ...
                      * (tsl.unit_price + COALESCE(tsl.item_tax,0))
                    ) AS val
                ")
                ->value('val') ?: 0;
        } else {
            // Value sold units at the original purchase cost
            $salesValue = DB::table('transaction_sell_lines_purchase_lines_v2 AS tspl')
        ...
        // C) STOCK ADJUSTMENTS
        //
        // Use variation’s default_purchase_price for cost mode,
        // or its sell_price_inc_tax for sale‐price mode
        $adjustExpr = $by_sale_price
            ? 'v.sell_price_inc_tax'
            : 'v.default_purchase_price';
    
        $adjustValue = DB::table('stock_adjustment_lines AS sal')
        ...
        // Net stock value (monetary)
        return (float) ($purchaseValue + $salesValue + $adjustValue);
    }
    /**
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = TransactionSellLine::join('transactions as sale', 'transaction_sell_lines.transaction_id', '=', 'sale.id')
            ->leftjoin('transaction_sell_lines_purchase_lines_v2 as TSPL', 'transaction_sell_lines.id', '=', 'TSPL.sell_line_id')
            ->leftjoin(
                'purchase_lines as PL',
                'TSPL.purchase_line_id',
                '=',
        ...
        $query->select(
            ...
            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)) AS total
                FROM transaction_sell_lines AS tsl
                    JOIN transaction_sell_lines_purchase_lines_v2 AS tspl2
        ...
        if (!empty($start_date) && !empty($end_date) && $start_date != $end_date) {
            $query->where('sale.trans_date', '>=', $start_date)
                ->where('sale.trans_date', '<=', $end_date);
        }
        if (!empty($start_date) && !empty($end_date) && $start_date == $end_date) {
            $query->where('sale.trans_date', $end_date);
        }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        //Zatca Code
        //If Transaction is Sent to Zatca It can't be Deleted
        $is_zatca_enabled = app(\App\Utils\ModuleUtil::class)->isModuleInstalled('Zatca');
        if (!empty($is_zatca_enabled)) {
            if ($transaction && $transaction->sent_to_zatca == 1) {
                $output = [
                    'success' => false,
                    'msg' =>  __('zatca::lang.transaction_sent_to_zatca_cannot_be_deleted')
                ];
                return $output;
            }
        }
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft' || $transaction->sub_type == 'none') {
                ...
                foreach ($transaction_payments as $payment) {
                    ...
                    //Delete payment because no foreign key constraint to cascade on delete transaction
                    $payment->delete();
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        ...
        //Zatca Code
        $is_zatca = app(\App\Utils\ModuleUtil::class)->isModuleInstalled('Zatca');
        if ($is_zatca) {
            $sells->addSelect(
    ...
    {
        // Initialize variables and fetch business ID
        $business_id = request()->session()->get('user.business_id');
        
        // Fetch previous transaction sums before the start date
        ...
        // Fetch previous payments before the start date
        $prev_payments = $this->__paymentQuery($contact_id, $start)
            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 'is_advance')
            ->get();
        
        // Calculate sums for previous payments
        $prev_total_advance_payment = $prev_payments->where('is_advance', 1)->sum('amount');
        $prev_total_regular_payments = $prev_payments->where('is_advance', 0)->sum('amount');
        ...
        $total_prev_paid = $prev_total_invoice_paid + $prev_total_purchase_paid - $prev_total_sell_return_paid - $prev_total_purchase_return_paid + $prev_total_ob_paid;
        $total_prev_advance_payment = $prev_total_advance_payment;
        ...
        // Process each transaction to build the ledger
        ...
        // Calculate sums for transactions
        ...
        $payments = $this->__paymentQuery($contact_id, $start, $end)->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 't.ref_no', 't.invoice_no')->get();
    
        // Fetch payment types for display
        $paymentTypes = $this->payment_types(null, true, $business_id);
    
        // Process each payment to build the ledger
        foreach ($payments as $payment) {
            if ($payment->transaction_type == 'sell' && $payment->is_return == 0) {
                $total_invoice_paid_amounts[] = $payment->amount;
            }
            if ($payment->transaction_type == 'purchase' && $payment->is_return == 0) {
                $total_purchase_paid_amounts[] = $payment->amount;
            }
            if ($payment->transaction_type == 'sell_return') {
                $total_sell_return_paid_amounts[] = $payment->amount;
            }
            if ($payment->transaction_type == 'purchase_return') {
                $total_purchase_return_paid_amounts[] = $payment->amount;
            }
            if ($payment->transaction_type == 'opening_balance') {
                $opening_balance_paid += $payment->amount;
            }
    
            // **Remove or Comment Out the Following Lines to Include Payments with `parent_id`:**
            // if (!empty($payment->parent_id)) {
            //     continue;
            // }
    
            // **Handle Advance Payments Appropriately:**
            ...
            if ($payment->is_advance == 1 && empty($ref_no)) {
                // **Advance Payment Not Applied to Any Invoice**
                $type = __('lang_v1.advance_payment');
                $debit = '₨0.00';    // Advance payments should not affect debit
                $credit = '₨0.00';   // Advance payments should not affect credit
                $advance = $payment->amount; // Populate advance field
                $note .= '<small>' . __('lang_v1.advance_payment') . '</small>';
            } elseif ($payment->is_advance == 1 && !empty($ref_no)) {
                // **Advance Payment Applied to an Invoice**
                $type = __('lang_v1.advance_payment_applied');
                $debit = $payment->amount;    // Debit to reflect application of advance
                $credit = '₨0.00';            // No credit for applying advance
                $advance = '';                // Advance is now applied
                $note .= '<small>' . __('lang_v1.advance_payment_applied') . '</small>';
            } else {
                ...
                $credit = (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance']) && $payment->is_return ==0) ? $payment->amount : '';
                $advance = '';
            }
    
            // **Include Payments with `parent_id` as 'Advance Payment Applied'**
            if (!empty($payment->parent_id)) {
                // Fetch the parent advance payment for reference if needed
                $parent_advance = TransactionPayment::find($payment->parent_id);
                $note .= '<small>' . __('lang_v1.advance_payment_applied') . ': ' . ($parent_advance ? $parent_advance->payment_ref_no : '') . '</small>';
            }
        ...
        // Prepend Opening Balance to the ledger
        $ledger = array_merge([[
            'date' => $start,
            'ref_no' => '',
        ...
        foreach ($ledger as $key => $val) {
            ...
            if ($val['type'] == __('lang_v1.advance_payment')) {
            ...
            $total_invoice_paid_amounts = [];
            $total_purchase_paid_amounts = [];
            $total_sell_return_paid_amounts = [];
            $total_purchase_return_paid_amounts = [];
        // Prepare output
        $output = [
            'ledger' => $ledger,
            ...
            'total_advance_payment' => $total_prev_advance_payment,
            // DEBUG
            'total_invoice_paid_amounts' => $total_invoice_paid_amounts,
            'total_purchase_paid_amounts' => $total_purchase_paid_amounts,
            'total_sell_return_paid_amounts' => $total_sell_return_paid_amounts,
            'total_purchase_return_paid_amounts' => $total_purchase_return_paid_amounts,
        ];
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        if (!empty($start_date) && !empty($end_date)) {
            if ($start_date == $end_date) {
                $sales_by_subtype->where('trans_date', $end_date);
            } else {
                $sales_by_subtype->whereBetween(DB::raw('trans_date'), [$start_date, $end_date]);
            }
        }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if (empty($sell_return)) {
            ...
            if (!empty($is_zatca_enabled)) {
                $sell_return_data['invoice_type'] = '383';
            }
    ...
    /**
     * Update the mark_as_sent column to mark the invoice as sent
     *
     * @param int $transaction_id
     *
     * @return string
     */
    public function markAsSent($transaction_id)
    {
        Transaction::where('id', $transaction_id)
            ->update(['email_sent' => 1]);
