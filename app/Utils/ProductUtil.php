...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        if ($product->product_type == 'combo') {
         ...
         // Fetch modifiers
         $product->modifiers = $product->product->modifier_sets;
         // Concatenate product and variation names
         $product->full_name = $product->product_actual_name . ' ' . $product->variation_name;
 ...
 public function getVariationGroupPrice($variation_id, $price_group_id, $tax_id)
    ...
    public function adjustStockOverSelling($transaction)
    {
        ...
        TransactionSellLinesPurchaseLines::where('purchase_line_id', 0)
                ->join('transaction_sell_lines as tsl', 'tsl.id', '=', 'transaction_sell_lines_purchase_lines_v2.sell_line_id')
                ...
                ->select('transaction_sell_lines_purchase_lines_v2.*')
                ->get();
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id, $adjust_current_stock = false)
    {
        ...
        if ($adjust_current_stock == 'true') {
            $qty_available = ($purchase_details->total_purchase + $purchase_details->total_opening_stock + $sell_details->total_sell_return + $purchase_details->total_purchase_transfer) - ($sell_details->total_sold + $purchase_details->total_adjusted + $purchase_details->total_purchase_return + $sell_details->total_sell_transfer);

            $update_current_stock = VariationLocationDetails::where('variation_id', 
                                            $variation_id)
                                        ->where('location_id', $location_id)
                                        ->update(['qty_available' => $qty_available]);
        }
