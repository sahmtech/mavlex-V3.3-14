...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        if (!empty($location)) {
            ...
            if (!empty($business_id)) {
                ...
                $custom_labels = Business::where('id', $business_id)->first(['custom_labels'])->custom_labels;
                $custom_labels = json_decode($custom_labels, true);
            } else {
                $custom_labels = [];
            }
    ...
    public function format_date($date, $show_time = false, $business_details = null)
    {
        ...
        if (!empty($show_time)) {
         ...
         return !empty($date) ? \Carbon::parse($date, $timeZone)->format($format) : null;
    }
    ...
    public function formatPhoneNumber($phone)
    {
        if (strlen($phone) >= 11) {
            ...
            $verifyPhone = substr($phone, 0, 1);

            if ($verifyPhone == '+') {
                $formatedPhone = substr($phone, 1);
            } else if ($verifyPhone == '0') {
                $formatedPhone = "234" . substr($phone, 1);
            }
    ...
    public function uploadFile($request, $file_name, $dir_name, $file_type = 'document')
    {
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            $allowed_extensions = config('constants.media_upload.allowed_extensions');
    
            if (!in_array($extension, $allowed_extensions)) {
                throw new \Exception("Invalid file extension. Allowed extensions are: " . implode(', ', $allowed_extensions));
            }
     ...
     * @return string
     */
    public function uploadFileWallet($request, $file_name, $dir_name, $file_type = 'document')
    {
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= config('constants.document_size_limit')) {
                $new_file_name = time() . '_' . rand() . "." . $request->file($file_name)->extension();
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{business_logo}') !== false) {
                ...
                $business_logo = !empty($logo_name) ? '<img src="' . upload_asset('uploads/business_logos/' . $logo_name) . '" alt="Business Logo" >' : '';
    ...
    public function parseNotifications($notifications)
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (in_array($notification->type, [\App\Notifications\RecurringInvoiceNotification::class, \App\Notifications\RecurringExpenseNotification::class, \App\Notifications\InvoiceReminderNotification::class])) {
                $msg = '';
                $icon_class = '';
                $link = '';
                if (
                    ...
                    $msg = __(
                        'lang_v2.invoice_reminder',
                        ['transaction' => $data['transaction_id']]
                    );
                    $icon_class = "fas fa-recycle bg-yellow";
                    $link = action('SellController@previewInvoice', [$data['transaction_id']]);
                }
    ...
    public function createUser($request)
    {
        ...
        if ($user_details['user_type'] == 'user') {
            ...
            if (!$moduleUtil->isSubscribed($business_id)) {
                ...
                return $moduleUtil->quotaExpiredResponse('users', $business_id);
            }
        }
