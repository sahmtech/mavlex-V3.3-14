...
use App\Models\Menu;
use App\Services\SidebarBuilder;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        //
        $this->app->singleton('moduleUtil', function ($app) {
        ...
        try {
            if (\Schema::hasTable((new AppSetting)->getTable())) {
                View::composer(['*'], function ($view) {
                    ...
                    $view->with('app_settings', $service->all());
                });
            }
        } catch (\Exception $e) {
            // migrations not run yet, skip sharing app_settings
        }
    }
    ...
    public function boot()
    {
        ...
        app(AppSettingsService::class)->apply_storage_config();
        ...
        //If module is present checks from module util module side bar - available installed and enabled(superadmin subscription)
        Blade::if('moduleEnabled', function ($module) {
            $moduleUtil = resolve('moduleUtil');
            $businessId = auth()->check() ? auth()->user()->business_id : null;
            $isEnabled = $businessId ? $moduleUtil->moduleSidebar($businessId, $module) : false;
            return $isEnabled;
        });
        ...
        View::share('app_settings', new AppSetting());

        // 2) If the table exists, overwrite with your real settings
        try {
            if (Schema::hasTable((new AppSetting)->getTable())) {
            ...
            // migrations not run yet — keep the default AppSetting()
        }
        // View::composer(['*'], function($view) {
        //     $service = app(AppSettingsService::class);
        //     $view->with('app_settings', $service->all());
        ...
        View::composer('*', function ($view) {
            ...
            $view->with('is_zatca', $moduleUtil->isModuleInstalled('Zatca'));
        });
        ...
        View::composer('menus.partials.*', fn($view) => $view->with('menus', MenuService::sidebar(Auth::id() ?: 0)));
