...
class ZATCASimplifiedTaxInvoice
{
    ...
    {
        $populated_template = require module_path( '/public/templates/simplified_tax_invoice_template.php');
    ...
    {
        $populated_template = require module_path('/public/templates/invoice_billing_reference_template.php');
    ...
    public function createInvoiceDigitalSignature(string $invoice_hash, string $private_key)
    {
        ...
        $binary_signature = base64_encode(openssl_get_privatekey($invoice_hash_bytes, $wrapped_private_key_string));
    ...
    {
        $populated_template = require module_path('/public/templates/ubl_signature_signed_properties_for_signing_template.php');
    ...
    {
        $populated_template = require module_path('/public/templates/ubl_signature_signed_properties_template.php');
    ...
    public function defaultUBLExtensions(string $invoice_hash, string $signed_properties_hash, string $digital_signature, string $cleanUpCertificateString, string $ubl_signature_signed_properties_xml_string): string
    {
        ...
        $populated_template = require module_path('/public/templates/ubl_signature.php');
    ...
    private function parseLineItems(array $line_items)
    {
        ...
        array_map(function ($line_item) use (&$total_taxes, &$total_subtotal, &$total_discount, &$invoice_line_items) {
            ...
            $total_discount += (float)$line_item_totals['discount'];
            #
...
//        }
         ...
         * set invoice lines
         */
        $tax_total_template = require module_path('/public/templates/tax_total_template.php');
         ...
         */
        $legal_monetary_total_template = require module_path('/public/templates/legal_monetary_total_template.php');

        $constructLegalMonetaryTotal = $this->constructLegalMonetaryTotal($total_subtotal, $total_taxes, $total_discount);
         ...
         * set invoice lines
         */
        $invoice_line_template = require_once module_path('/public/templates/invoice_line_template.php');
    ...
    private function constructLegalMonetaryTotal(float $total_subtotal, float $total_taxes, float $total_discount)
    {
        ...
        return [
            ...
            'cbc:AllowanceTotalAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => number_format($total_discount, 2, '.', '')
            ],
            'cbc:PrepaidAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => 0
    ...
    private function constructTaxTotal(array $line_items)
    {
        ...
        array_map(function ($line_item) use (&$addTaxSubtotal, &$taxes_total) {
            $total_line_item_discount = array_reduce($line_item['discount'], function ($p, $c) {
                return $p + $c['amount'];
            }, 0);
