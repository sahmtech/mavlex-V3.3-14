...
class EGS
{
    ...
    private function generateCSR(string $solution_name, $private_key)
    {
        ...
        if (!is_dir(public_path('tmp/'))) {
            mkdir(public_path('tmp/'), 0775);
        }

        $private_key_file_name = public_path('tmp/') . self::uuid() . '.pem';
        $csr_config_file_name = public_path('tmp/') . self::uuid() . '.cnf';

        $private_key_file = fopen($private_key_file_name, 'w');
        $csr_config_file = fopen($csr_config_file_name, 'w');

        require module_path('public/templates/csr_template.php');
    ...
    private function defaultCSRConfig(string $solution_name)
    {
        ...
        $template_csr = require module_path( '/public/templates/csr_template.php');
