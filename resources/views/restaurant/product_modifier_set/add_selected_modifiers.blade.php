...
<small>
	@foreach($modifiers as $modifier)
		...
		(<span class="modifier_price_text">{{@num_format($modifier->sell_price_inc_tax)}}</span> X <span class="modifier_qty_text">{{@num_format($quantity)}}</span>)
...
@else
	@foreach($modifiers as $modifier)
		<div class="product_modifier">
		<div class="modifier_name"> {{$modifier->variations->name ?? ''}} </div>
		(<span class="modifier_price_text">{{@num_format($modifier->unit_price_inc_tax)}}</span> X <span class="modifier_qty_text">{{@num_format($modifier->quantity)}}</span>)
		<input type="hidden" name="products[{{$index}}][modifier][]" 
