...
<input type="hidden" name="discount_cd [{{$row_count}}]" id="discount_cd" value="{{$discount_amount}}">
		...
		<small class="text-muted p-1">{{ number_format($product->qty_available, 2) }} {{$product->unit }} @lang('lang_v1.in_stock')</small>
		@else
	...
	<td class="{{$hide_tax}}">
		...
		<input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" style="height: 30px; line-height: 30px;" class="form-control input-sm pos_unit_price_inc_tax input_number" value="{{@num_format($unit_price_inc_tax)}}" @if(!$edit_price) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$unit_price_inc_tax}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)])}}" @endif>
	</td>
	...
	<td class="text-center">
		...
		@endphp
		<input type="{{$subtotal_type}}" style="height: 30px; line-height: 30px;" class="form-control input-sm pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax )}}">
