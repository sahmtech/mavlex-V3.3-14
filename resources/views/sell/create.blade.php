...
<style>
	...
	.pos_quantity.input_number.mousetrap.input_quantity {
		width: 80px !important;
		text-align: center;
		font-size: 16px;
		border: 1px solid #ccc 0,1,0,1;
...
<div class="main-container no-print">
	...
	<div class="storys-container">
		@if(!empty($status) && $status == 'quotation')
		@include('layouts.partials.sub_menu.quotation', ['link_class' => 'sub-menu-item'])
		@else
	...
	<div class="card-wrapper">
		<div class="overview-filter">
			<div class="title">
				<h1>@lang('lang_v2.add_invoice')</h1>
			</div>
			<div class="flex-17">
				@if(count($business_locations) > 0)
				<div class="form-box">
					{!! Form::select('select_location_id', $business_locations, $default_location->id ?? null, ['class' => 'form-control',
					'id' => 'select_location_id', 'required', 'autofocus', 'style' => 'width: 260px'], $bl_attributes); !!}
				</div>
				@endif

				<button type="button" class="customize-btn btn-modal"
					data-href="{{action('SellController@customize')}}"
					data-container=".customize_modal">
					<img src="{{ asset('img/icons/settings.svg') }}" alt="Customize" />
					Customize
				</button>
			</div>
		</div>
		...
		<div class="accordionItem">
			...
			<div class="accordionTitle is-open">
				<h2 class="">@lang('lang_v1.sell_details')</h2>
				...
				<div class="row">
					...
					<div class="form-box">
						...
						<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
							...
							<small>
								<div class="bill-to-option">
									<button type="button">
										@lang('lang_v2.bill_to'):
						...
						{{--<div class="form-box hide">
							{!! Form::label('due_date', __('sale.due_date') . ':*') !!}
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-calendar"></i>
								</span>
								{!! Form::text('due_date', $default_date, ['class' => 'form-control', 'readonly', 'required', 'id' => 'user_dob']); !!}
							</div>
						</div>--}}
						<div class="clearfix"></div>
						<!-- Sales Order -->
						...
						<div class="col-sm-3">
							<div class="form-group">
								...
								{!! Form::select('sales_order_ids[]', [], null, ['class' => 'form-control select2 input-sm', 'multiple', 'id' => 'sales_order_ids']); !!}
							</div>
						</div>
						@endif
						<!-- Attach Documents -->
						...
						<div class="col-sm-3">
							<div class="checkbox" style="display: inline-flex; align-items: center;">
								<label style="margin-right: 10px;">
									{!! Form::checkbox('is_recurring', 1, false, ['class' => 'input-icheck', 'id' => 'is_recurring']); !!} @lang('lang_v1.subscribe')?
								</label>
								<button type="button" data-toggle="modal" data-target="#recurringInvoiceModal" class="btn-link" style="display: inline; padding: 0; border: none;">
				...
				<div class="row">
					...
					<div class="row col-sm-12 pos_product_div" style="min-height: 0">
						...
						<div class="table-responsive" style="min-height: .01%; max-height:600px; overflow-x: auto; overflow-y:auto">
								<table class="table table-condensed table-bordered table-striped table-responsive max-table" id="pos_table">
								<thead>
									<tr>
								...
								<tbody class="invoice-tbodt"></tbody>
							</table>
						</div>
		...
		<div class="accordionItem">
			<!-- Accordion Title -->
			<div class="accordionTitle is-open">
				<h2 class="">@lang('lang_v1.totals')</h2>
				<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M6 9L12 15L18 9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</div>
		...
		<div class="accordionItem">
			<!-- Accordion Title -->
			<div class="accordionTitle is-open">
				<h2 class="">@lang('sale.shipping_details')</h2>
				<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M6 9L12 15L18 9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</div>
			...
			<div class="accordionContent">
				<div class="row">
					...
					<div class="col-md-12 text-center">
						<button type="button" class="btn bg-yellow" id="toggle_additional_expense"> <i class="fas fa-plus"></i> @lang('lang_v1.add_additional_expenses') <i class="fas fa-chevron-down"></i></button>
					</div>
		...
		<!-- Export Details Accordion-->
		<div class="accordionItem">
			<!-- Accordion Title -->
			<div class="accordionTitle is-open">
				<h2 class="">@lang('lang_v1.is_export')</h2>
				<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M6 9L12 15L18 9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</div>
		...
		@can('sell.payments')
		<!-- Payment Account Accordion-->
		<div class="accordionItem">
			<!-- Accordion Title -->
			<div class="accordionTitle is-open">
				<h2 class="">@lang('lang_v1.payment_accounts')</h2>
				<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M6 9L12 15L18 9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</div>
		...
		@if(empty($status) || !in_array($status, ['quotation', 'draft']))
			...
			<div class="accordionItem">
				<!-- Accordion Title -->
				<div class="accordionTitle is-open">
					<h2 class="">@lang('lang_v1.payment')</h2>
					<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M6 9L12 15L18 9" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
					</svg>
				</div>
				...
				<div class="row">
					...
					<div class="invoice-footer">
						<button type="button" id="submit-sell" class="save-btn">@lang('messages.save')</button>
						<button type="button" id="save-and-print" class="save-record-payment">@lang('lang_v1.save_and_print')</button>
					</div>
				</div>
		...
		<div class="modal fade customize_modal" tabindex="-1" role="dialog"
		...
		<script type="text/javascript">
			...
			$("#enable_staff").click(function() {
				...
				if (checked) {
					$('div.is_enable_div').removeClass('hide');

				} else {
					$('div.is_enable_div').addClass('hide');
				}

			});

			//pay and add payment
			$('button#save-and-record-payment').click(function() {
			...
			$('#modal_payment').one('shown.bs.modal', function() {
				//set is save and record payment to 1 once modal is opened.
				$('#is_save_and_record_payment').val(1);
			});

			$('#modal_payment').on('hide.bs.modal', function(e) {
				//set is save and record payment to 0 once modal is closed.
				$('#is_save_and_record_payment').val(0);
			});
		</script>
		@endsection